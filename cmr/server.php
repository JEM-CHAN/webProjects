    ...
    if(!empty($title) && !empty($author)){ 
        // Validate email 
            ...
            if($uploadStatus == 1){ 
                ...
                $sqlQ = "INSERT INTO files (author, title, date_submission, adviser, abstract, keywords, file_name) VALUES (?,?,?,?,?,?,?)"; 
                $stmt = $db->prepare($sqlQ); 
                $stmt->bind_param("sssssss", $author, $title, $date_submission, $adviser, $abstract, $keywords, $uploadedFile); 
                $insert = $stmt->execute(); 
...
if(isset($_POST['user_search'])) {
    ...
    if(mysqli_num_rows($SQL_AUTH_SEARCH_QUERY) > 0) {
        while($ROWS_AUTH_SEARCH_QUERY = mysqli_fetch_assoc($SQL_AUTH_SEARCH_QUERY)) {
            ...
            <td><center>".$ROWS_AUTH_SEARCH_QUERY['date_submission']."</center></td>
